...
enum Cmd {
    ...
    /// Native/profile agents: catalog, blocking run, durable background spawn
    Agent {
        #[command(subcommand)]
        cmd: AgentCmd,
    },
...
#[derive(Subcommand)]
enum AgentCmd {
    /// Inventory launchable coding tools, native profiles, providers, packages
    Catalog {
        #[arg(long)]
        json: bool,
    },
    /// Run a native/profile agent turn in the foreground
    Run {
        #[arg(long, default_value = "default")]
        profile: String,
        ...
        #[arg(long)]
        session: Option<String>,
        /// Require that this package is visible to the profile before running.
        #[arg(long = "with-package")]
        with_packages: Vec<String>,
        /// Prompt text, or '-' to read stdin through exec.
        prompt: String,
    },
    /// Queue a native/profile agent turn for the daemon to run
    Spawn {
        #[arg(long, default_value = "default")]
        profile: String,
        ...
        #[arg(long, default_value_t = 0)]
        priority: i64,
        /// Require that this package is visible to the profile before spawning.
        #[arg(long = "with-package")]
        with_packages: Vec<String>,
        /// Prompt text.
        prompt: String,
