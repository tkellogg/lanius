...
/// workdir (a headless worker's whole job is its workdir) plus the per-session
/// generated CODEX_HOME (codex >=0.142 must write there at startup), network =
/// loopback
...
fn codex_headless_cage(root: &Root, workdir: &Path, codex_home: &Path) -> crate::sandbox::Cage {
    // Canonical write roots: SBPL `subpath` rules and the Protect fence match the
    // real (inode) path, so a symlinked workdir component would not bind.
    let write_root = workdir
        .canonicalize()
        .unwrap_or_else(|_| workdir.to_path_buf());
    // The per-session CODEX_HOME must be writable too: codex >=0.142 writes into
    // its home at startup (PATH aliases, the in-process app-server client's
    // state) and exits 1 under a read-only home. The home is per-session and
    // generated by us; the auth symlink inside it still resolves to the user's
    // real ~/.codex, which stays OUTSIDE the write roots (SBPL matches the
    // canonical target), so this widens nothing but the session's own scratch.
    let home_root = codex_home
        .canonicalize()
        .unwrap_or_else(|_| codex_home.to_path_buf());
    crate::sandbox::Cage::from_roots_with_policy(
        vec![write_root, home_root],
        vec![],
        true,
...
fn run_codex_capture(
    ...
    let cage = codex_headless_cage(root, workdir, &codex_home);
...
"#;
    ...
    /// M1: the headless codex cage targets ONLY the session workdir and the
    /// per-session CODEX_HOME for writes (codex >=0.142 writes into its home at
    /// startup) and runs the loopback network policy — reachable bus/local read
    /// planes, external egress cut. Portable (no enforcement needed to inspect
    /// the shape).
    #[test]
    fn codex_headless_cage_targets_workdir_and_loopback() {
        ...
        let codex_home = root.dir.join("run/code-t/codex_home");
        ...
        // The write roots are the canonical workdir + CODEX_HOME — nothing wider.
        // (The cage normalizes root order, so compare as sets.)
        let mut expect = vec![
            workdir.canonicalize().unwrap(),
            codex_home.canonicalize().unwrap(),
        ];
        expect.sort();
        let mut got = cage.write_roots.clone();
        got.sort();
    ...
    fn codex_headless_cage_fences_writes_live() {
        ...
        let codex_home = root.dir.join("run/code-t/codex_home");
        std::fs::create_dir_all(&codex_home).unwrap();
        let cage = codex_headless_cage(&root, &root.dir, &codex_home);
        ...
        let home_ok = cage
            .shell_command(&format!("echo hi > {}/alias.txt", codex_home.display()))
            .output()
            .unwrap();
        assert!(
            home_ok.status.success(),
            "write inside CODEX_HOME must succeed: {home_ok:?}"
        );
    ...
    fn codex_headless_cage_posture_only_for_headless_codex() {
        ...
        assert_eq!(p["write"], "workdir+codex-home");
