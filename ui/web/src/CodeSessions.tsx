...
import WorkerNoteCompose from './components/WorkerNoteCompose';
...
export default function CodeSessions({ focus, onOpenConversation }: { focus?: string; onOpenConversation?: (noun: string, session: string) => void } = {}) {
  ...
  // chrome-polish M4 → worker-notes-panel: the "send a note to this worker"
  // compose is now the SHARED <WorkerNoteCompose>, mounted here on the runs detail
  // AND in the trace fallback (ConverseView) — one component, no divergence. It
  // relays through `lanius code deliver` (POST /api/code/deliver) into the
  // worker's INBOX; never a chat message, never the conversation projection.
      ...
      {detail && (() => {
        ...
        <div className="cs-detail">
          ...
          {/* worker-notes-panel: the shared compose (same component the trace
              ...
              a running job's inbox, not a chat. Feedback is the CLI's own
