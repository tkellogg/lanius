...
A cron sweep (no resident process) that re-surfaces unanswered `human/ask`
events as `signal/attention` — which the notify package renders like any
other signal. Stops when: an answer arrives, the deadline default fires, a
`delivery/<channel>/acked` receipt names the ask (read receipts from
